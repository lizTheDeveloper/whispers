...
- ADVANCE THROUGH LOCATIONS: If the world state lists multiple known locations, the party should visit NEW locations as the story progresses — don't let them linger in corridors or side rooms for more than 2-3 rounds. Each scene transition should move deeper into the adventure. If the party has been in the same location for 3+ rounds, create a reason to move them forward (a collapsing passage, a discovered exit, an NPC leading them onward).
