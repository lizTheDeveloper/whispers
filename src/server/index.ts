...
wss.on('connection', (ws) => {
  ...
  ws.on('close', () => {
    ...
    if (players) {
      ...
      if (players.length === 0) {
        const jc = currentJoinCode;
        // Nobody is at the table: stop spending on it now, not after the
        // teardown grace. Paused (not stopped) so it is persisted and the
        // host can resume it — the teardown below may still drop the loop,
        // and resume rebuilds it from the checkpoint. A deliberate host pause
        // keeps its reason; a quiet pause does not, because once the loop is
        // torn down a whisper can no longer lift it — only the host can.
