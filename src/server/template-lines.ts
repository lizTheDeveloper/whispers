...
export function invokeLines(name: string, aspect: string): string[] {
  return [
    ...
    `${name} remembers what "${aspect}" means, and the problem shrinks.`,
    `A little "${aspect}" goes a long way: ${name} turns it around.`,
    `"${aspect}", through and through — that is how ${name} wins the moment.`,
    `${name} spends a fate point on "${aspect}", and it is worth it.`,
    `Nobody does it quite like "${aspect}" — and ${name} proves it.`,
    ...
    `${name} lets "${aspect}" lead, and a way opens up.`,
    `One glance says this is a job for "${aspect}". ${name} delivers.`,
    `"${aspect}" tips the balance, and ${name} comes out ahead.`,
  ];
}
...
export function compelLines(name: string, trouble: string): string[] {
  return [
    ...
    `${name} sighs. "${trouble}" — every single time. Still, fate is keeping a tab.`,
    `"${trouble}" makes things harder for ${name}, and the tale rewards the honesty.`,
    `The old story of "${trouble}" plays out once more; ${name} gains a token of fate.`,
    `${name} can't resist: "${trouble}". A small setback, and fate tucks a reward away.`,
    `Here comes "${trouble}", right when ${name} least needs it — fate makes it up to ${name}.`,
    `"${trouble}" leads ${name} astray for a beat; the luck from it will come in handy.`,
    `${name}'s old friend "${trouble}" drops by uninvited, and leaves a small gift of fate behind.`,
  ];
}
