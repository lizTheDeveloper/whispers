...
/**
 * A closing reflection as the table sees it. It is broadcast to everyone
 * (the "[Final reflection]" action line), so it is public: live (Z9JKG2)
 * Biz's said "I do not know if the voice that told me to stay next to Mom
 * was right to trust." The clause about the voice goes (the same filter as
...
export class GameLoop {
  ...
  private isChild(character: Character): boolean {
    return childrenInParty(this.partyForDm()).some(n => this.namesMatch(n, character.definition.name));
  }

  /** The tone judge; tests swap in a mock. Undefined: the LLM judge on the game's proxy. */
  static toneJudge: ToneJudge | undefined;
  /** The one-call judge for the child's options; tests swap in a mock. Undefined: the LLM judge. */
  static toneListJudge: ToneListJudge | undefined;
  ...
  /**
   * The delivered ruling without the server's stock beats — each as appended,
   * or as guardText left it — for the memory writers.
   */
  private memoryText(delivered: string, beats: string[]): string {
    if (beats.length === 0) return delivered;
    const variants = beats.flatMap(b => [b, this.guardText(b)]);
    return withoutStockBeats(delivered, variants);
  }
  ...
  /**
   * consistentProse as a pause-safe loop step: a pause during the check
   * parks and redoes only the check, never the DM call before it. Null when
  ...
  private addressTermsOf(speakerId: string): AddressTerm[] {
    ...
    if (!speaker) return [];
    const own = (speaker.definition.relationships ?? []).flatMap(r => {
    ...
    // "mother" with no address term on the sheet: "Mom, Liz" still collapses.
    const companions = Array.from(this.characters.values()).filter(o => o.id !== speakerId).map(o => o.definition.name);
    return [...own, ...kinAddressTerms(speaker.definition.relationships ?? [], companions)];
  }
  ...
  private async runOpening(campaign: any): Promise<void> {
    ...
    const opening: DmOpening | null = await this.haltable<DmOpening | null>(async () => this.toneGated('opening',
      ...
      { mapText: (o, edit) => ({ ...o, arrival: o.arrival ? edit(o.arrival) : o.arrival, narration: o.narration ? edit(o.narration) : o.narration, introductions: o.introductions?.map(x => ({ ...x, text: x.text ? edit(x.text) : x.text })) }) },
    ), (e) => {
    ...
    if (sceneText && !scenarioOpening) {
      const checked = await this.checkedProse(sceneText);
    ...
    for (const id of this.state.initiativeOrder) {
      ...
      if (!c) continue;
      const text = await this.checkedProse(this.introductionFor(c, opening));
      if (text === null) return;
  ...
  private withArrival(sceneText: string, opening: DmOpening | null, premise: string): string {
    ...
    if (narration && narratesTransport(narration) && partyWord.test(narration)) {
      ...
      const pronouns = Array.from(this.characters.values()).map(c => this.ownPronouns(c));
      lead = [fallbackArrival(premise, names, pronouns), dmArrival].filter(Boolean);
    }
  ...
  private async runScene(campaign: any): Promise<void> {
    ...
    const narrateGated = async (pacing: typeof narrateArgs.pacing & { repeatedBeat?: string }) => this.toneGated('narration',
      ...
      { mapText: (n, edit) => ({ ...n, narration: edit(n.narration) }) },
    );
    ...
    // A sentence already said (an NPC's line, a described image) comes out
    // of an otherwise new beat; a beat that is all repeats is handled whole below.
    {
      const fresh = withoutRepeatedSentences(narration.narration, recentBeats);
      if (fresh) narration.narration = fresh;
    }
    if (repeatsRecentBeat(narration.narration, recentBeats)) {
      ...
      if (again && checkedAgain && !repeatsRecentBeat(checkedAgain, recentBeats)) {
        narration.narration = withoutRepeatedSentences(checkedAgain, recentBeats) || checkedAgain;
  ...
  private async processTurn(characterId: string, campaign: any): Promise<void> {
    ...
    // Options are in the character's own voice: "I tell Mom Liz…" is "I tell Mom…".
    const ownTerms = this.addressTermsOf(characterId);
    ...
    // The character's own report of how it took the whisper, checked against
    // what it actually did (see whisper-verdict.ts).
    if (whisper) {
      const checked = checkedWhisperVerdict(whisper, decision.chosenAction, decision.spokenWords, decision.whisperedInfluence);
      if (checked !== decision.whisperedInfluence) {
        console.log(`[game-loop] ${character.definition.name} said it followed the whisper, but "${decision.chosenAction.slice(0, 60)}" shares nothing with it — counted as partial`);
        decision.whisperedInfluence = checked;
      }
    }
    ...
    // Round 16 (NUMMRL). No game mechanics in what a character thinks or
    ...
    const resolveGated = async (...args: ResolveArgs) => this.toneGated('ruling',
      ...
      // Round 16 (NUMMRL): kept while still flagged, the flagged phrases come
    ...
    // The server's own lines appended to the ruling this turn (corrections,
    // asides, invoke and compel beats): the table reads them, the memory
    // writers do not (round 16 — "The worry about Biz flared up again" was a
    // compel line, stored as Liz's memory).
    const stockBeats: string[] = [];
    if (diceResult && resolution.difficulty != null && resolution.skill && campaign.system_id === 'fate-core') {
      ...
      if (resolution.outcome !== correctOutcome) {
        ...
        if (dmSaid === 'success' && (correctOutcome === 'tie' || correctOutcome === 'success-with-cost' || correctOutcome === 'failure')) {
          ...
          if (beats.length > 0) {
            const beat = this.lines.pick(`correction-${correctOutcome}`, beats, this.recentStoryText());
            resolution.narration = appendBeat(resolution.narration, beat);
            stockBeats.push(beat);
          }
        }
      }
    ...
    if (campaign.system_id === 'fate-core' && character.state.fatePoints >= 2 && !fpSpentByDm && !fpAlreadySpentThisTurn) {
      if (resolution.outcome === 'tie' || resolution.outcome === 'success-with-cost') {
        ...
        if (invokeLine) {
          resolution.narration = appendBeat(resolution.narration, invokeLine);
          stockBeats.push(invokeLine);
        }
    ...
    for (const cid of affectedCharIds) {
      ...
      if (c && c.state.stress >= 3 && c.state.consequences.length >= 2) {
        const takenOutMsg = takenOutLine(c.definition.name, this.ownPronouns(c));
    ...
    if (resolution.outcome !== 'failure') {
      ...
      for (const other of this.characters.values()) {
        ...
        const namesForThem = [getFirstName(other.definition.name), other.definition.name, ...this.addressTermsOf(characterId).filter(t => t.name === other.definition.name && !t.derived).map(t => t.address)];
    ...
    if (this.state.currentTurn - lastCompelTurn >= 3) {
      ...
      // A compel is the trouble happening (round 16, NUMMRL: "Wanders off
      // after anything shiny… the words could be Biz's motto" on the turn Biz
      // ignored the shiny glint). Shown in what they did or said or in the
      // ruling: the compel and its line. Not shown: no compel — except the
      // economy's proactive point at 0 FP, which comes with no line that
      // claims the trouble happened.
      const shown = shouldCompel && troubleShown(character.definition.trouble,
        [decision.chosenAction, decision.spokenWords, withoutStockBeats(resolution.narration, stockBeats)],
        Array.from(this.characters.values()).map(c => c.definition.name));
      if (shouldCompel && !shown && !proactive) {
        console.log(`[game-loop] Compel skipped: "${character.definition.trouble}" did not show in ${character.definition.name}'s turn`);
      } else if (shouldCompel) {
        ...
        if (shown) {
          ...
          console.log(`[game-loop] Fate point for the economy: ${character.definition.name} now at ${character.state.fatePoints} FP — "${character.definition.trouble}" did not show, so no compel line`);
          this.addTranscript('system', `[${character.definition.name} earns a fate point (${character.state.fatePoints} FP)]`);
        }
    ...
    // A sentence the DM already said is not said again (live: the scene
    // opening's NPC line and its mist, word for word in the next ruling).
    // A ruling that repeats an earlier beat whole (see runScene) is not read
    // twice: its mechanics stand, told in one plain line.
    const freshRuling = withoutRepeatedSentences(resolution.narration, this.recentDmBeats());
    if (freshRuling) resolution.narration = freshRuling;
    else if (resolution.narration.trim()) resolution.narration = this.plainOutcome(character, resolution.outcome);
    ...
    // What memory reads: the ruling as the table read it, without the stock beats.
    const rulingForMemory = this.memoryText(resolution.narration, stockBeats);
    this.memoryStore.extractAndStore(
      characterId, this.campaignId, character.definition.name,
      decision.chosenAction, rulingForMemory, whisper,
    ...
    for (const [observerId, observer] of this.characters) {
      ...
      this.memoryStore.storeObservation(
        ...
        character.definition.name, decision.chosenAction, rulingForMemory,
        this.state.currentScene, this.state.currentTurn,
  ...
  private async maybeCompactTranscript(): Promise<void> {
    ...
    try {
      ...
      return; // paused or stopped mid-check: the next completed turn compacts
    }
    if (this.stopped || this.pauseReason) return;
    this.transcript = [
  ...
  private async writeEpilogue(): Promise<void> {
    ...
    const charLines = Array.from(this.characters.values()).map(c => {
      // The epilogue is read by the whole table: no whisper memories and no
      // trust in "the voice" go into it (live, a reflection said "the voice
      // that told me to stay next to Mom" aloud).
      const memories = this.memoryStore.recall(c.id, 6).filter(m => m.type !== 'whisper').slice(0, 3);
      const memText = memories.map(m => m.content).join('. ');
      const carrying = c.state.inventory?.length ? `carrying ${c.state.inventory.join(', ')}` : 'carrying nothing';
      return `${c.definition.name} (${c.definition.highConcept}): ${currentCondition(c.state)}, ${carrying}, stress ${c.state.stress}/3, ${c.state.fatePoints} FP. Key memories: ${memText || 'none'}`;
    }).join('\n');
  ...
  /**
   * Where the items are and where the party went, as the world bible last
   * recorded it — the facts the ending got wrong live ("now the Golden Seal
   * is ours" after it was handed over; an atrium put in the wrong building).
   */
  private endingFacts(): string {
    const { items, places, openThreads } = this.worldBible.getEndingFacts(this.campaignId);
    return [
      items.length > 0 ? `\n\nItems, latest known whereabouts:\n${items.map(i => `- ${i}`).join('\n')}` : '',
      places.length > 0 ? `\n\nPlaces the party went (what each one is):\n${places.map(p => `- ${p}`).join('\n')}` : '',
      openThreads.length > 0 ? `\n\nStill unresolved when the session ended (leave these open):\n${openThreads.map(t => `- ${t}`).join('\n')}` : '',
    ].join('');
  }
  ...
  private async generateCharacterClosingReflections(scenes: Array<{ scene_number: number; summary: string }>, currentScene: string, facts = '', epilogue = ''): Promise<void> {
    // The epilogue already says how things ended (and what stayed open); the
    // reflections must agree with it — live, Liz's claimed a hand-over the
    ...
    for (const [charId, char] of this.characters) {
      ...
      // Both lines are broadcast to the table, so the whisper is never named
      // to the model here (live, Biz's reflection said "the voice that told
      // me to stay next to Mom" aloud): only how the character feels about
      // their own choices.
  ...
  handleWhisper(text: string, sender: { characterId: string | null; isOwner: boolean }): WhisperAck {
    ...
    if (queue.length >= GameLoop.WHISPER_QUEUE_LIMIT) {
      return {
        ...
        message: whisperInboxMessage(target.definition.name, this.ownPronouns(target), 'full'),
      };
    }
    queue.push(text);
    ...
    return {
      ...
      message: whisperInboxMessage(target.definition.name, this.ownPronouns(target), 'queued'),
    };
  }
  ...
  private knownNpcNames(): string[] {
    try {
      return (this.db.prepare('SELECT name FROM entities WHERE campaign_id = ? AND type = ?').all(this.campaignId, 'npc') as Array<{ name: string }>)
        .map(r => r.name).filter(n => !this.isPartyName(n));
    } catch {
      return [];
    }
  }
  ...
  /**
   * A name that is really a party member: their name, or their high concept
   * recorded as an NPC by an older fact extraction ("Curious Kid With a
   * Sketchbook" — which a chip then called "Curious").
   */
  ...
  private extractNpcNamesFromNarration(narration: string): string[] {
    ...
    ).all(this.campaignId, 'npc') as Array<{ name: string }>).filter(n => !this.isPartyName(n.name));
    return npcs
      // The name as a word, as written ("Pell"), not any substring ("curious" in "a curious hush").
      .filter(n => new RegExp(`(?<![\\w'’-])${getFirstName(n.name).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?![\\w'’-])`).test(narration))
  ...
  private addTranscript(role: TranscriptMessage['role'], content: string, characterId?: string): void {
    ...
    // An NPC's pronouns are fixed by the first DM beat that shows them, and
    // the DM is handed them every turn after (getSummary). Live, Marni went
    // from "their spectacles" to "her saucer eyes".
    if (role === 'dm') {
      try {
        this.worldBible.learnNpcPronouns(this.campaignId, guarded, Array.from(this.characters.values()).map(c => c.definition.name));
      } catch (e) {
        console.error('[world-bible] NPC pronoun check failed:', e);
      }
    }
  }
}
...
/**
 * The first sight of the world is read by one of the players, before they
 * have said who they are: the players' characters are never named in it.
 ...
 */
export function worldIntroductionAsShown(text: string, seed: WorldSeed, gentlePeril: boolean, partyNames: string[] = []): string {
  if (!text?.trim()) return text;
  let out = readerNotNamed(text, partyNames);
  try {
